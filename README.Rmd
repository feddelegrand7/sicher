...
[![CRAN status](https://www.r-pkg.org/badges/version/sicher)](https://CRAN.R-project.org/package=sicher) [![R-CMD-check](https://github.com/feddelegrand7/sicher/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/feddelegrand7/sicher/actions/workflows/R-CMD-check.yaml)
...
Prevents any `NA` from entering a typed variable — a common source of silent bugs in data pipelines. The underlying type is validated first; `NonNA` adds an extra NA-free guarantee on top.
...
Accepts numeric values within the closed interval `[min, max]`. Every element of a vector must satisfy the bounds. `NA` values are always rejected.
...
Validates every element of a character vector against a Perl-compatible regex. The pattern is compiled eagerly at construction time, so typos in the regex are caught immediately. An empty `character(0)` passes vacuously; pair with `NonEmpty()` to also require at least one element.
...
Rejects zero-length vectors, strings, and lists. For data frames it checks `nrow > 0`. The base type is validated first, then the length is checked.
...
Use `extend()` when a new schema should reuse an existing list type instead of redefining every field:
...
If `extra` reuses a field name from the base type, the new field definition overrides the old one and `extend()` emits a warning so the change is explicit.
