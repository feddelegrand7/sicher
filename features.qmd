...
Reuse a structured list type and add fields on top of it:
...
`extend()` preserves all required base fields, carries optional fields forward, and adds the new ones from `extra`.
...
If you intentionally redefine a field, `extend()` warns and the new type takes precedence:

```{r}
DetailedPerson <- suppressWarnings(extend(Person, list(
  age = Integer
)))
